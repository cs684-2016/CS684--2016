/****************************************************/
// Filename: main.c
// Created By: Nishant Kathpal
// Creation Date: 24-10-2016
// Purpose/Description: Contains functions to execute
//	the task
/****************************************************/
...
#define IP_ADDR         0xC0A8010A    //need2change
//#define IP_ADDR         0x0A811C57
...
static _i32 BsdTcpClient(_u16 Port,unsigned char test[512]);
...
int main(int argc, char** argv)
{
	...
	init_done = f_mount(&sdVolume, "", 1);	//Initialise MMC/SD Card and store status in init_done variable
...
static _i32 BsdTcpClient(_u16 Port,unsigned char test[512] )
{
    ...
    for (idx=0 ; idx<512 ; idx++)
        {
            uBuf.BsdBuf[idx] = (_u8)(test[idx]);
        //    uBuf.BsdBuf[idx] = buffer[idx];
        }

    for (idx=512 ; idx<BUF_SIZE ; idx++)
    {
...
static void modeSelection(){
	...
	char iptoconnect[15];
	int p = 0;
	UARTprintf("Following Modes Are Available : \n");
	UARTprintf("A) Drive 2 Drive Direct Connection : \n");
	UARTprintf("B) Drive 2 Drive Over Local Router : \n");
	UARTprintf("Enter Mode : \n");
	while(UARTCharsAvail(UART0_BASE) == false)
	{
	}
	cChar = (char)UARTCharGet(UART0_BASE);
	UARTprintf("Selected Mode : %c \n",cChar);
